...
source("C:/Workspace/matrixOperators.r")
...
# for debugging change warnings to errors 
...
determine.destination <- function(depart,emigrnts,adjMove,dist.vec,pr.pred) { 
...
sbf_sim <- function(input, dir.nm, rowNum) {
  ...
  ## fertility data 
  clutch.size.lr.NOTUSED <- c(1885,3893) # L. raniformis
  ...
  tomet.dur.iter <- round(sum(rnorm(1, hatch.dur, hatch.dur.sd.prop), rnorm(1, tadpole.dur, tadpole.dur*tadpole.dur.sd.prop)), 0)
  ...
  ad.s.yr.sd <- 0.05 # Litoria aurea (Pickett et al.)
  ...
  hat <-  rnorm(1,hatch.pr,hatch.pr.sd)
  if (hat < 0) { hat <- 0 }
  if (hat > 1) { hat <- 1 }
  tads <- rnorm(1,tadpole.mn,tadpole.sd)
  if (tads < 0) { tads <- 0 }
  if (tads > 1) { tads <- 1 }
  
  tomet.s.iter <- hat * tads
  
  ads <- rnorm(1, ad.s.yr.mn, ad.s.yr.sd)
  if (ads < 0) { ads <- 0 }
  if (ads > 1) { ads <- 1 }
  toad.daily.s.iter <- nthroot(ads, 365)
  # trial run just use mn
  ...
  for (s in 1:5) {
    ads <- rnorm(1, ad.s.yr.mn, ad.s.yr.sd)
    # can add the while loop if this is problematic
    if (ads < 0) { ads <- 0 }
    if (ads > 1) { ads <- 1 }
    ad.s.vec.iter[s] <- ads  }
  ...
  K.rel.tad.dem.vec <- c(100, K.rel.tad.Hold2, K.rel.tad.Hold3, K.rel.tad.Hold4)
  ...
  K.rel.juv.dem.vec <- c(200, K.rel.juv.Hold2, K.rel.juv.Hold3, K.rel.juv.Hold4)
  ...
  # this isnt used in the sensitivity analysis
  totalSurv <- sum(popmat[2:6,1:6])
  for (size in 1:4) {
    for (x in 1:6) { init.vec[x,size] <- 0 }
    fert <- -500
    while (fert < 0) { fert <- rnorm(1,clutch.size.lr,clutch.size.lr.sd.prop*clutch.size.lr)}
    
    init.vec[1,size] <- round(initSpawn[size] * fert)
  ...
  ## dims are wetlands, decades, iterations (0 dead, 1 alive)
    ...
    for (e in 1:iter) {
      ...
      for (i in 1:(generations)) {
        ...
        for (wetlands in 1:wetlandNum) {
          ...
          min.clutch.size.lr <- max.clutch.size.lr <- -500
          while (min.clutch.size.lr < 0) {  min.clutch.size.lr <- as.numeric(quantile(rnorm(n=1000, mean=clutch.size.lr, sd=clutch.size.lr.sd.prop*clutch.size.lr), probs=0.025, na.rm=T))}
          while (max.clutch.size.lr < 0) { max.clutch.size.lr <- as.numeric(quantile(rnorm(n=1000, mean=clutch.size.lr, sd=clutch.size.lr.sd.prop*clutch.size.lr), probs=0.975, na.rm=T))}
          if (min.clutch.size.lr > max.clutch.size.lr) {
            min.clutch.size.lr <- min.clutch.size.lr + max.clutch.size.lr
            max.clutch.size.lr <- min.clutch.size.lr - max.clutch.size.lr
            min.clutch.size.lr <- min.clutch.size.lr - max.clutch.size.lr
          }
          fert.iter <- round((runif(stages, min.clutch.size.lr, max.clutch.size.lr)) * prop.breeding, 0)
          ...
          if (runNum %% 5 == 1) { 
            ...
            if (antecedentWet[wetlands] == T && stayWet[1,wetlands] < annualWetness[e,i-1]) {
              popmat.current[1,] <- popmat.current[1,]/fishImpact
            }
            #Wet nat
            ...
            if (wetlands < 3) {
              f <- (i %% 3) + 1
              if (banrockFish[f] == T) { popmat.current[1,] <- popmat.current[1,]/fishImpact }
              n.mat[,i+1,wetlands] <- popmat.current %*% n.mat[,i,wetlands]
          ...
          if (totalPop > 0) {
            ...
            # cuayuela 16 +/- 14
            PrMove.iter <- -500
            while (PrMove.iter < 0) { PrMove.iter <- rnorm(1,PrMove,0.14) } 
            for (potMov in 1:totalPop) { 
              if (runif(1,0,1) < PrMove.iter) { emigrants <- emigrants + 1 }
            }
          }
          
          if (emigrants >= 1) {
            # Use the determine destinations function to determine destinations        
            destinations <- determine.destination(wetlands,emigrants,adj.wetlandMovement,dist.vec,pr.pred)
  ...
  PrSvAll <- sum(occupancyHeatC[,7,1:iter])
  
  # save
  input$PrSvAll <- PrSvAll
  save.nm <- paste0('res3',sprintf("%09.0f", rowNum))
  assign(save.nm, input)
  save(list=save.nm,file=paste(dir.nm,save.nm,sep='/'))
  
 
  
  #last line of the sbfsim function
}

## parameter ranges
ranges <- list()
ranges$clutch.size.lr <- c(1500,6000)
ranges$hatch.dur <- c(1, 5)
ranges$hatch.pr <- c(0.5, 0.98)
ranges$tadpole.dur <- c(50, 90)
ranges$tadpole.mn <- c(0.03, 0.5)
ranges$ad.s.yr.mn <- c(0.05, 0.35)
ranges$K.rel.tad.Hold2 <- c(150, 750)
ranges$K.rel.tad.Hold3 <- c(400, 1500)
ranges$K.rel.tad.Hold4 <- c(500, 3000)
ranges$K.rel.juv.Hold2 <- c(100, 700)
ranges$K.rel.juv.Hold3 <- c(300, 1250)
ranges$K.rel.juv.Hold4 <- c(500, 2000)
ranges$moveSurv <- c(0.1, 0.6)
ranges$PrMove <- c(0.1, 0.6)
ranges$max.dist <- c(5000,50000)
ranges$fishImpact <- c(100,600)
...
# write.csv(lh, file = "G:/My Drive/MetapopLH.csv", row.names = F)

lh <- read.csv("G:/My Drive/MetapopLH.csv")
# lh <- lh[1:10,]
...
#  res <- foreach(rowNum=1:nrow(lh),.verbose=T) %do% {sbf_sim(input=lh[rowNum,],dir.nm=dir.nm,rowNum=rowNum)}
 res <- foreach(rowNum=3000:3999,.verbose=T) %do% {sbf_sim(input=lh[rowNum,],dir.nm=dir.nm,rowNum=rowNum)}
# run not in parallel
# for (xx in 1:nrow(lh)) {  sbf_sim(input=lh[xx,],dir.nm=dir.nm,rowNum=xx) }
...
dat <- data.frame(rbindlist(res.list), fill = T)
...
#  for(i in 1:10) {
  nm <- res.nms[i]
  aa <- load(paste0(dir.nm,nm))
  bb <- eval(as.name(aa))
  if (length(bb) > 17)  {
  bb <- subset(bb, select = -c(iter))
  save(bb,file=paste(dir.nm,nm,sep='/'))
  }
}
...
#dat.nona$cllminP <- cloglog(dat.nona$minP)


brt.fit <- gbm.step(dat.nona, gbm.x = attr(dat.nona, "names")[1:16], gbm.y = attr(dat.nona, "names")[17], family="poisson", max.trees=200000, 
